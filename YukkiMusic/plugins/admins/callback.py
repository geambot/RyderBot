...
from RyderBot.misc import SUDOERS, db
from RyderBot.utils.database import (is_active_chat,
...
from RyderBot.utils.formatters import seconds_to_min
from RyderBot.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await RyderBot.pause_stream(chat_id)
        ...
        await RyderBot.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await RyderBot.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                await RyderBot.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await RyderBot.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
